...
def test_register_login_roundtrip(client):
    assert client.post("/auth/register", json={"trainee_id": "Alice!"}).status_code == 201
    assert client.post("/auth/register", json={"trainee_id": "alice"}).status_code == 409
    login = client.post("/auth/login", json={"trainee_id": "alice"})
    assert login.status_code == 200
    assert login.json()["trainee_id"] == "alice"
    assert login.json()["token_type"] == "bearer"
    assert client.post("/auth/login", json={"trainee_id": "ghost"}).status_code == 404
...
def test_cross_user_isolation_with_real_jwt(tmp_path, monkeypatch):
    ...
    try:
        with TestClient(app) as api:
            alice = api.post("/auth/register", json={"trainee_id": "alice"}).json()
            bob_token = api.post("/auth/register", json={"trainee_id": "bob"}).json()["access_token"]
...
def test_login_rate_limit(client):
    client.post("/auth/register", json={"trainee_id": "alice"})
    statuses = [client.post("/auth/login", json={"trainee_id": "alice"}).status_code for _ in range(7)]
